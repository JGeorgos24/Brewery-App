import React, {Component} from "react";


class BeerList extends Component{
    constructor(props) {
        super(props);

        this.state={
            BeerContainer: [
                {
                    state: ''
                }
            ]
        }
    }


    render(){
        // console.log(this.props);
        return(
            <div>
                <p>{this.props.beer}</p>
                <button onClick={() => this.props.handleAdd(this.props.beerId, true)}>Add to my profile</button>
                <p>Upvotes: {this.props.beer.upvotes}</p>
                <img className="small" src="data:image/png;base64," 
                onClick={() => this.props.handleUp(this.props.beerId, this.props.beer.upvoteState)} />
                <img className="small" src="https://www.clipartkey.com/mpngs/m/95-950756_hd-th-lets-upvote-transparent-background-red-down.png" 
                onClick={() => this.props.handleDown(this.props.beerId, this.props.beer.downvoteState)} />
            </div>
        )
    }
}

export default BeerList;
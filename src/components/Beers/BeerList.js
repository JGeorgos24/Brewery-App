import React, {Component} from "react";


class BeerList extends Component{
    constructor(props) {
        super(props);

        this.state={
            BeerContainer: [
                {
                    state: ''
                }
            ]
        }
    }


    render(){
        return(
            <div className="BeersList">
                <p className="BeerTitle">{this.props.beer.name}</p>
                {this.props.loggedIn && <button onClick={() => this.props.handleAdd(this.props.beerId, true)}>Add to my profile</button>}
                <div className='beerVote'>
                    <p>Upvotes: {this.props.beer.upvotes}</p>
                    {this.props.loggedIn && 
                        <img className="small" src="data:image/png;base64," 
                        onClick={() => this.props.handleUpBeer(this.props.beerId, this.props.beer.upvoteState)} />
                     }
                    {this.props.loggedIn && 
                            <img className="small" src="https://www.clipartkey.com/mpngs/m/95-950756_hd-th-lets-upvote-transparent-background-red-down.png" 
                        onClick={() => this.props.handleDownBeer(this.props.beerId, this.props.beer.downvoteState)} />
                    }   
                </div>
               
            </div>
        )
    }
}

export default BeerList;